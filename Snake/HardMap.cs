...
namespace Snake
{
    public partial class HardMap : Form
    {
        public HardMap()
        {
            InitializeComponent();
        ...
        Image vatcan = Properties.Resources.woodbox;
            ...
            for (int i = Snake.Count - 1; i >= 0; i--) //Duyệt từ đuôi, xuống else để hiểu
            {
                if (i == 0) //Trường hợp duyệt tới đầu
                {
                    ...
                    for (int j = 0; j < rects.Count; j++)
                        if (ChamChuongNgai(rects[j], Snake[i]))
                        {
        ...
        private void pictureBox1_Paint(object sender, PaintEventArgs e) //Vẽ trên picturebox
        {
            ...
            {
                canvas.DrawImage(vatcan, x * 16, 7 * 16, 32, 32);
                canvas.DrawImage(vatcan, x * 16, 23 * 16, 32, 32);
            }
            for (int x = 23; x <= 37; x += 2)
            {
                canvas.DrawImage(vatcan, x * 16, 7 * 16, 32, 32);
                canvas.DrawImage(vatcan, x * 16, 23 * 16, 32, 32);
            }
            for (int y = 9; y < 17; y += 2)
            {
                canvas.DrawImage(vatcan, 5 * 16, y * 16, 32, 32);
                canvas.DrawImage(vatcan, 37 * 16, y * 16, 32, 32);
            }
            for (int y = 17; y < 23; y += 2)
            {
                canvas.DrawImage(vatcan, 5 * 16, y * 16, 32, 32);
                canvas.DrawImage(vatcan, 37 * 16, y * 16, 32, 32);
            }
        ...
        private void HardMap_Load(object sender, EventArgs e)
        {
            RestartGame();
        }
        ...
        private void RestartGame()
        {
            ...
            highScore = Modify.TaiKhoans(query)[0].hardPoint;
            ...
            for (int x = 5; x <= 19; x += 2)
            {
                rects.Add(new Rectangle(x * 16, 7 * 16, 32, 32));
                rects.Add(new Rectangle(x * 16, 23 * 16, 32, 32));
            }
            for (int x = 23; x <= 37; x += 2)
            {
                rects.Add(new Rectangle(x * 16, 7 * 16, 32, 32));
            ...
            for (int y = 9; y < 17; y += 2)
            {
                rects.Add(new Rectangle(5 * 16, y * 16, 32, 32));
                rects.Add(new Rectangle(37 * 16, y * 16, 32, 32));
            }
            for (int y = 17; y < 23; y += 2)
            {
                rects.Add(new Rectangle(5 * 16, y * 16, 32, 32));
        ...
        private void GameOver()
        {
            ...
            if (score > highScore)
            {
                string query = "UPDATE TaiKhoan SET HardPoint = " + score + " WHERE TenTK = '" + Settings.Instance.TenTK + "'";
